...
const MyBookings = () => {
  ...
  // 1. Fetch Data (Thêm timestamp để tránh cache và lỗi CORS)
  const fetchMyBookings = async () => {
    try {
      setLoading(true);
      const timestamp = new Date().getTime();
      ...
      if (data.success) {
        // Lọc bỏ các booking bị lỗi (null)
        const validBookings = (data.bookings || []).filter(booking => 
  ...
  const canEditBooking = (booking) => {
    if (!booking) return false;
    // Chỉ cho sửa nếu chưa thanh toán và chưa hủy
  ...
  const canCancelBooking = (booking) => {
    ...
    // RULE: Nếu chưa thanh toán (Pending) -> Cho hủy thoải mái
    if (!booking.isPaid) return true;

    // RULE: Nếu đã thanh toán (Confirmed) -> Check 24h
    const now = new Date();
    const checkIn = new Date(booking.checkIn);
    
    if (now >= checkIn) return false; // Quá ngày checkin
    
    const hoursDiff = (checkIn - now) / (1000 * 60 * 60);
    if (hoursDiff < 24) return false; // Sát giờ quá (dưới 24h)
